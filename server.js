...
mongoClient.connect(process.env.url,cp,function(err,name){
	...
	console.log("Database connected.");
	  ...
	  app.get("/mainPost", function (request, result) {
		result.render("mainPost");
	...
	//   app.get("/detelePost",function(request,result){
	// 	result.render("deletePost");
	//   });
	  ...
	  app.post("/getNewsfeed", function (request, result) {
		...
		// 			postdata.push(user[i].posts)
		// 		}
        //   //var postid =[]
		// 		for(var j=0;j<postdata.length;j++){
		// 			ids.push(postdata[i]._id);
		// 		}
		// 	//	ids.push(user._id);
   ...
   app.post("/getNewspost", function (request, result) {
	var accessToken = request.fields.accessToken;
	database.collection("users").findOne({
		"accessToken": accessToken
	}, function (error, user) {
   ...
   app.post("/getprofileposts", function (request, result) {
	var accessToken = request.fields.accessToken;
	database.collection("users").findOne({
		"accessToken": accessToken
	}, function (error, user) {
...
app.post("/deletepost", function (request, result) {
	...
	app.post("/deleteProfile", function (request, result) {
		 var accessToken = request.fields.accessToken;
